"""
app/repo/pronunciation_repo.py

CRUD helpers for UserPronunciationProfile.

BUGS FIXED vs original:
- update_profile() used (old_avg + new_score) / 2 which is a two-sample
  mean and gets increasingly wrong as exercise count grows.
  Replaced with a running weighted mean:
      new_avg = (old_avg * (n-1) + new_score) / n
- Renamed from `prounciation_repo.py` (typo) to `pronunciation_repo.py`.
"""
import logging
