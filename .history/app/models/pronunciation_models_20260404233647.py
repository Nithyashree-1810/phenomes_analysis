...
ORM models for the pronunciation analysis domain.

BUG FIXED:
- UserPronunciationProfile.id is the internal PK (Integer autoincrement).
- user_id stores a client-supplied integer that must be unique.
- The relationship join used the non-PK column user_id — this is valid
  but must be explicit; kept as-is because the DB schema already works
  this way.
"""
