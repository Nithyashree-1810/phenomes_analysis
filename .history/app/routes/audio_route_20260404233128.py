...
POST /test/analyze — Accept audio upload, transcribe, score, persist.

BUGS FIXED vs original:
1. The old route had a large dead-code block (old version in triple-quoted
   string).  Removed.
2. Profile was updated twice in the old code: once manually in the route
   and once via post_exercise_hook.  Consolidated: profile creation/update
   uses get_or_create_profile + post_exercise_hook.
3. update_phoneme_stats was called BEFORE the profile existed in the old
   'else' path, causing FK violations on first inserts.
   FIX: get_or_create_profile is called first; it flushes the profile row
   so the FK can be satisfied before phoneme upserts.
4. Temp files were never deleted on success or failure.  Added cleanup in
   a finally block.
5. Errors returned a 200 with {"error": "..."}.  Changed to raise
   HTTPException so clients get proper status codes.
6. Added request_id (UUID) to every response for tracing.
"""
import logging
import os
import uuid
...
from app.repo.phoneme_performance_repo import get_or_create_profile
...
async def analyze_audio(
    ...
    user_id: int = Query(..., description="Caller-supplied integer user ID"),
