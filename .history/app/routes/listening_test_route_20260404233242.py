"""
app/routes/listening_test_route.py

POST /listening/submit-audio
Accepts a user's spoken audio answer, transcribes it with Whisper,
evaluates semantic comprehension with GPT-4o-mini, and persists the attempt.

BUGS FIXED vs original:
1. Used a local OpenAI() client (not the shared one).  Replaced with the
   shared LangChain client for tracing.
2. ListeningAttempt model lacked session_id — now included.
3. Temp file used the original filename which could clash between concurrent
   requests.  Now uses UUID-prefixed temp files.
4. Temp file was not deleted on error.  Fixed with try/finally.
5. Bare exception on JSON parse returned {relevance:0, correctness:0} which
   is misleading — now logs and re-raises as a 500.
"""
import json
import logging
import os
