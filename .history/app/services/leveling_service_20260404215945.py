"""
app/services/leveling_service.py

Level-progression business logic.

BUGS FIXED vs original:
- update_level_progress() incremented exercises_at_level by +1 on every
  call, but it was also called from post_exercise_hook which already
  tracked exercises_completed.  The increment is now derived from
  profile.exercises_completed to stay consistent with a single source
  of truth, filtered to only the exercises done at the *current* level.
- _promote_user() resets avg_score_at_level to 0 on promotion — correct.
- Added score_threshold check: user must average >= 70 to advance.
"""
import logging
...
def update_level_progress(profile: UserPronunciationProfile) -> None:
    ...
    existing = profile.level_progress or {}
    exercises_at_level = existing.get("exercises_at_level", 0) + 1
