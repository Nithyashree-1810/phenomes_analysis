...
Generates listening comprehension passages, TTS audio, and questions.

BUGS FIXED vs original:
- listening_service.py and listening_question_service.py were DUPLICATE
  implementations of the same functionality with slightly different class
  structures.  Consolidated into one module used by all routes.
- Used gpt-3.5-turbo which is deprecated/legacy.  Upgraded to gpt-4o-mini.
- All LLM calls now go through LangChain ChatOpenAI for LangSmith tracing.
- question_prompt.py referenced {difficulty} but generate_questions_from_passage
  never passed it.  Fixed: difficulty is now forwarded to the prompt.
"""
import json
import logging
import re
