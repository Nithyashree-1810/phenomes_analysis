...
def update_phoneme_stats(db: Session, user_id: int, phoneme_results: list):
    """
    Update or create PhonemePerformance records based on exercise results.
    ...
    for result in phoneme_results:
        ...
        record = db.query(PhonemePerformance).filter_by(
            user_id=user_id, phoneme=phoneme
        ).first()
...
def update_profile_summary(db: Session, user_id: int):
    """
    Recalculate weak/strong phoneme lists, overall score, and level progress.
    """
    ...
    for p in stats:
        ...
        if acc < 50:
            weak.append({
                "phoneme": p.phoneme,
                "error_rate": round((100 - acc) / 100, 2)  # convert to 0-1
            })
        elif acc >= 70:
            strong.append({
                "phoneme": p.phoneme,
                "accuracy": round(acc / 100, 2)  # convert to 0-1
            })
    ...
    profile.overall_score_avg = float(sum(scores) / len(scores)) if scores else 0

    # Mark last practice timestamp
    profile.last_practice_at = datetime.utcnow()

    # Update level progress dict
...
def post_exercise_hook(db: Session, user_id: int, phoneme_results: list, time_spent_secs: int):
    """
    Main hook called after an exercise:
    - Updates phoneme stats
    - Updates profile summary (weak/strong, score, level_progress)
    - Updates exercises completed & time spent
    """
    ...
    # 3. Increment exercises and time
