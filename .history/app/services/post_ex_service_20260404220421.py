"""
app/services/post_ex_service.py

Post-exercise hook: updates phoneme stats and profile aggregates.

BUGS FIXED vs original:
- update_phoneme_stats() was duplicated (called both directly in
  audio_route AND inside post_exercise_hook).  Consolidated: the route
  uses post_exercise_hook; the hook calls update_phoneme_stats internally.
- recompute_weak_strong_and_score() used a simple (a+b)/2 rolling average
  which is wrong for large sample sizes.  Now uses a proper weighted mean:
    new_avg = (old_avg * (n-1) + new_score) / n
  Actually, the easiest correct approach for our use-case is to recompute
  from phoneme accuracy rows, which is what this function already does —
  kept as-is since it reads live data.
- Removed the orphaned duplicate call in audio_route (see audio_route.py).
"""
import logging
